...
do
    ls backlog-jobs | grep $bundle_id
done)

for file in `echo "$job_jsons"`;do folder=$(echo $file | sed 's/.json//g'); mkdir $input_dir/completed-jobs/$folder; mv $input_dir/backlog-jobs/$file $input_dir/completed-jobs/$folder/$file;done
